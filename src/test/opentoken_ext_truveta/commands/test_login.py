...
from opentoken_ext_truveta.auth import AuthError, Credentials, write_session_api_url
from opentoken_ext_truveta.commands.initiate_exchange import _initiate_exchange
from opentoken_ext_truveta.commands.login import DEFAULT_DOMAIN_URL, _login
...
class TestLoginCommand:
    def test_returns_zero_on_success(self):
        with (
            ...
            patch(
                "opentoken_ext_truveta.commands.initiate_exchange.ensure_auth",
                return_value=_fake_creds(),
            ),
            patch(
                "opentoken_ext_truveta.commands.initiate_exchange.load_or_generate_domain_keys",
                return_value=_sample_keypair(),
            ),
            patch(
                "opentoken_ext_truveta.commands.initiate_exchange.call_exchange_endpoint",
                return_value=_make_server_response(),
            ),
            patch(
                "opentoken_ext_truveta.commands.initiate_exchange.build_exchange_config",
                return_value={"payload": {"test": "data"}},
            ),
            patch(
                "opentoken_ext_truveta.commands.initiate_exchange.write_exchange_config",
    ...
    def test_prints_welcome_email_only_when_name_equals_email(self, capsys):
        with (
            patch(
                "opentoken_ext_truveta.commands.login.ensure_auth",
                return_value=_fake_creds(),
            ),
        ):
            _login(_args())
    ...
    def test_returns_one_on_key_management_error(self, capsys):
        from opentoken_ext_truveta.exchange.key_management import KeyManagementError
    ...
    def test_returns_one_on_exchange_api_error(self, capsys):
        from opentoken_ext_truveta.api.exchange import ExchangeAPIError
    ...
    def test_returns_one_on_exchange_config_error(self, capsys):
        from opentoken_ext_truveta.exchange.config import ExchangeConfigError
    ...
    def test_credentials_cached_even_when_exchange_fails(self, tmp_path, monkeypatch):
        ...
        def mock_ensure_auth(url, cached_only=False):
            from opentoken_ext_truveta.auth import _extract_domain, _write_cache
        ...
        with (
            ...
            patch(
                "opentoken_ext_truveta.commands.initiate_exchange._extract_service_domain",
    ...
    def test_round_trip_exchange_config_load(self, tmp_path, monkeypatch):
        ...
        from opentoken_ext_truveta.exchange.config import load_exchange_config
        ...
        with (
            ...
            patch(
                "opentoken_ext_truveta.exchange.config.decrypt_hashing_secret",
