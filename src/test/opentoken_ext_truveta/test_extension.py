...
from opentoken_ext_truveta.extension import TruvetaExtension
...
class TestRegisterSubcommand:
    ...
    def test_login_accepts_auth_domain_flag(self):
        root = self._build_parser()
        parsed = root.parse_args(
            [
                "truveta",
        ...
        assert parsed.domain == "http://localhost:8080"
        assert parsed.auth_domain == "https://api.dev.truveta-int.com"
...
class TestLoginDispatch:
    ...
    def test_login_returns_zero_on_success(self, capsys):
        ...
        with patch(
            "opentoken_ext_truveta.commands.login.ensure_auth", return_value=_FAKE_CREDS
        ):
...
class TestLogoutDispatch:
    ...
    def test_logout_returns_zero(self, tmp_path, monkeypatch):
        monkeypatch.setattr(
            "opentoken_ext_truveta.commands.logout.Path.home", lambda: tmp_path
        )
        args = MagicMock()
    ...
    def test_logout_reports_no_credentials(self, tmp_path, monkeypatch, capsys):
        monkeypatch.setattr(
            "opentoken_ext_truveta.commands.logout.Path.home", lambda: tmp_path
        )
        args = MagicMock()

        TruvetaExtension._logout(args)
